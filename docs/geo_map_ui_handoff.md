...
底部“地图” → “查看校园地图”。选择临时区域可高亮并自动聚焦；“演示位置前进一步”由父页面替换位置与轨迹参数，不创建任务、不使用定时器。障碍开关检查显示与隐藏。开发版可开启坐标拾取，点击地图空白处并复制“纬度, 经度”。release 构建不显示拾取面板，也不会触发拾取回调。
...
预览页现读取 CampusGeoMapData，不再使用 TemporaryGeoData；旧临时样例文件仅保留备查。引入2号 feature/campus-geo-engine 的7ed3910、1d85f0f、296f674、678b812四个提交。

当前提供6个地点：实验楼、第一食堂、第二教学楼、学生宿舍、图书馆、第一教学楼。前4个有演示路线；图书馆和一教没有路线，界面明确提示。第一教学楼只有中心点，没有区域边界，组件支持空polygon的中心点标记及点击回调，不补造边界。canteen_2、teaching_3、main_road尚无数据，不展示为已录入地点。

依据2号交接说明，实验楼和一教中心有公开地图核对记录，图书馆中心经人工核对；其他中心存在近似值。所有已提供polygon仍为近似矩形，路线中间点与充电点用于Demo，尚未完成道路或实地校准。

UI仍由外部参数驱动，预览页保留手动位置步进。已引入位置Adapter代码但尚未将定时模拟与任务、语音、安全控制接线。公开API不变，新增允许空polygon表示仅中心点。

接入后 flutter analyze --no-pub 通过；完整测试392项通过（含中心点区域选择回归检查）。首页在线文字的显示修正单独保留，不归入Geo接入提交。

## 校园地图入口调整

旧 CampusDemoPage 页面及入口已移除，原真实地理地图入口统一为“查看校园地图”，页面标题为“校园地图”。CampusMapView 底层组件和数据服务保留供兼容使用。AppShell 不再创建旧页面专用的定位模拟与协调器。A/B/C 任务轨迹展示仍保留，地理地图仍由外部数据驱动，未接业务控制。

技术文档见 technical/清穹_技术文档_校园地图版.docx。文档历史截图中的“真实地理地图”即当前“校园地图”。

## 2026年9月10日数据同步

已接入2号 feature/campus-geo-engine 的 d161816，本分支对应提交32c2412。CampusGeoMapData与该提交一致，地点由6个增至9个，新增第二食堂、第三教学楼、校园主干道。新增地点仅有Demo近似中心点，没有polygon或plannedPath；原6个地点、4条路线及充电点未修改。UI直接读取数据，新增地点可选择、定位，缺少路线及边界时显示说明。

本次验证：flutter analyze --no-pub无问题；flutter test --no-pub全部364项通过。此前文档和截图中“6个地点”为旧版本记录；当前以本节及campus_geo_handoff.md为准。地理预览仍未连接任务、安全或真实定位。

队长合并本分支后已包含d161816的数据更新，无需再重复挑选该提交。
