import '../campus_geo/campus_college_data.dart';
...
class CampusMapData {
  // Compatibility projection for the legacy normalized campus map.
  static CampusPoint _project(CampusGeoPoint point) => CampusPoint(
    x: (point.longitude - 121.88) / 0.03,
    y: (30.90 - point.latitude) / 0.03,
  );

  static final List<CampusZone> zones = List.unmodifiable([
    ...const <CampusZone>[
      CampusZone(
        id: 'lab_building',
        name: '实验楼',
        aliases: ['实验楼', '实验大楼', '实验楼附近', '实验区'],
    ...
    for (final zone in CampusCollegeData.zones)
      CampusZone(
        id: zone.id,
        name: zone.name,
        aliases: zone.aliases,
        center: _project(zone.center),
        polygon: List.unmodifiable(zone.polygon.map(_project)),
        category: CampusZoneCategory.teaching,
      ),
  ]);
  ...
  static CampusRoute routeForZone(String zoneId) {
    for (final route in CampusCollegeData.routes) {
      if (route.zoneId == zoneId) {
        return CampusRoute(
          zoneId: zoneId,
          plannedPath: List.unmodifiable(route.plannedPath.map(_project)),
        );
      }
    }
    switch (zoneId) {
      ...
      default:
        ...
        return CampusRoute(
          ...
          plannedPath: zone == null
              ...
              : [chargingStation.position, zone.center],
        );
    }
  }
}
